...
class Feature_extractor(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            return output
class T_FCN8s(nn.Module):
    def __init__(self, num_classes):
        super(T_FCN8s, self).__init__()
        vgg = models.vgg16(pretrained=True)
        features, classifier = list(vgg.features.children()), list(vgg.classifier.children())
    ...
    def forward(self, x, other_frame):
        ...
        merge_pool3 = torch.cat((output3, 3*pool3.unsqueeze(dim = 1)), dim = 1)
        merge_pool4 = torch.cat((output4, 3*pool4.unsqueeze(dim = 1)), dim = 1)
        merge_pool5 = torch.cat((output5, 3*pool5.unsqueeze(dim = 1)), dim = 1)
