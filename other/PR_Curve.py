...
def read_predict_GT_mask(predict_path, GT_path):
    ...
    for num_files in LISTDIR(predict_path):
        ...
        for dir_files in LISTDIR(G_full_path):
            G_mask_path = os.path.join(G_full_path, dir_files, "mask")
            for G_mask_files in tqdm(LISTDIR(G_mask_path)):
                img_GT_path = os.path.join(G_mask_path, G_mask_files)
...
def plot_curve(config):
    read_predict_GT_mask(config.GT_path)
