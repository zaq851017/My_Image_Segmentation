...
class Single_Nested_Unet(nn.Module):
    ...
    def forward(self, input):
        ...
        x3_0 = self.conv3_0(self.pool(x2_0))
        x2_1 = self.conv2_1(torch.cat([x2_0, F.upsample(self.up(x3_0), x2_0.size()[2:], mode='bilinear')], 1))
        x1_2 = self.conv1_2(torch.cat([x1_0, x1_1, self.up(x2_1)], 1))
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, self.up(x1_2)], 1))

        x4_0 = self.conv4_0(self.pool(x3_0))
        x3_1 = self.conv3_1(torch.cat([x3_0, F.upsample(self.up(x4_0), x3_0.size()[2:], mode='bilinear')], 1))
        x2_2 = self.conv2_2(torch.cat([x2_0, x2_1, F.upsample(self.up(x3_1), x2_1.size()[2:], mode='bilinear')], 1))
...
class Temporal_Nested_Unet(nn.Module):
    ...
    def forward(self, input, other_frame):
        ...
        output0 = torch.tensor([]).cuda()
        for i in range(image_num):
            temp = self.conv0_0(other_frame[:,i:i+1,:,:,:].squeeze(dim = 1))
            temp = temp.view(-1, 1, 32, 368, 424)
            output0  = torch.cat((output0, temp), dim = 1)
        temp_x0_0 = self.conv0_0(input)
        x0_0 = torch.cat((output0, temp_x0_0.unsqueeze(dim = 1)), dim = 1)
        x0_0 = torch.mean(x0_0, dim = 1)
        x1_0 = self.conv1_0(self.pool(x0_0))
        x0_1 = self.conv0_1(torch.cat([x0_0, self.up(x1_0)], 1))
